...
int sdl_display_init_bottom(uint16_t height, uint16_t width, uint16_t zoom_percent,
			    void **window, void **renderer, void **texture)
{
	*window = SDL_CreateWindow("Zephyr Display", SDL_WINDOWPOS_UNDEFINED,
				   SDL_WINDOWPOS_UNDEFINED, width * zoom_percent / 100,
				   height * zoom_percent / 100, SDL_WINDOW_SHOWN);
	if (*window == NULL) {
